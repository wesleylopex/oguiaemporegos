      ...
      <div class="h-full grid grid-cols-1 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 p-10 lg:p-0 flex items-center justify-center">
          <div class="xl:w-2/5 bg-white shadow-md rounded-sm p-6">
            <h1 class="text-gray-800 font-medium text-xl">Informe seu e-mail</h1>
            <p class="paragraph text-sm mt-1">Vamos enviar um link de recuperação de senha para o seu e-mail.</p>
            <form action="<?= base_url('forgotPassword/email/send') ?>" method="post" class="mt-6 grid grid-cols-1 gap-6">
              ...
              <div>
                <button class="w-full btn btn--primary">Enviar e-mail</button>
              </div>
            </form>
          </div>
        </div>
    ...
    async function sendForm (form) {
      try {
        ...
        if (!success) {
          ...
          window.location.href = '<?= base_url('esqueceu-senha/email-enviado?type=' . $this->input->get('type')) ?>'
        }
      } catch (error) {
